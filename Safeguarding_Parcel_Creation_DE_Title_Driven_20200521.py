...
gdb = r'C:\Users\ukdxe008\Desktop\Safeguarding_2b_2019_2020\Scripts\Draft_Scripts\HMLR_Driven_Script_DE_20200521\Working_Folders\Scratch2.gdb'
finished = r'C:\Users\ukdxe008\Desktop\Safeguarding_2b_2019_2020\Scripts\Draft_Scripts\HMLR_Driven_Script_DE_20200521\Working_Folders\SLOPs.gdb'
workspace=r'C:\Users\ukdxe008\Desktop\Safeguarding_2b_2019_2020\WSP_Working_Folder\M2M3_Amends\20200528\DE_SG_Amends_20200528.gdb'

fc1= r'C:\Users\ukdxe008\Desktop\Safeguarding_2b_2019_2020\WSP_Working_Folder\M2M3_Amends\20200528\DE_SG_Amends_20200528.gdb\STATUTORY_PROCESSES\LandOwnershipParcels'

fc2= r'C:\Users\ukdxe008\Desktop\Safeguarding_2b_2019_2020\WSP_Working_Folder\M2M3_Amends\20200528\DE_SG_Amends_20200528.gdb\STATUTORY_PROCESSES\HMLR_Parcels'

fc3 = r'C:\Users\ukdxe008\Desktop\Safeguarding_2b_2019_2020\WSP_Working_Folder\M2M3_Amends\20200528\DE_SG_Amends_20200528.gdb\STATUTORY_PROCESSES\relLandOwnership_HMLR'

fc4 = gdb + '\SLOP_Input_HMLR'

fc5 = gdb + '\SLOP_Input_HMLR_Temp'

fc20 = gdb + '\SLOP_Input_LOP'

fc21 = gdb + '\SLOP_Input_LOP_Temp'
...
fc30 = gdb+"\SG_New_Intersect_LOP"

fc31 = gdb+"\SG_New_Intersect_Dissolve_LOP"

fc32 = gdb+"\SG_Old_Intersect_LOP"

fc33 = gdb+"\SG_Old_Intersect_Dissolve_LOP"
...
fc15 = gdb+"\Registered_Safeguarding_Land_Ownership_Parcels_Draft"

fc25 = finished+"\Safeguarding_Land_Ownership_Parcels"
...
arcpy.AddField_management(fc2, 'Include', "TEXT","","","", "", "", "", "")
...
arcpy.Dissolve_management(fc2,fc12,"HMLR_Title_No")
...
print "Updating New Safegaurding and HMLR"
#Intersect and Dissolve the New Safegaurding Area
arcpy.Intersect_analysis([fc2,fc6],fc8)
arcpy.Dissolve_management(fc8, fc9,'HMLR_Title_No')

print "Updating Old Safegaurding and HMLR"
#Intersect and Dissolve the Old Safegaurding Area
arcpy.Intersect_analysis([fc2,fc7],fc10)
arcpy.Dissolve_management(fc10, fc11,'HMLR_Title_No')
...
print "Updating  HMLR Parcels required for safegaurding"
...
print"Updating Unique Field for HMLR"
...
#do the same with the LOPs

print "Updating New Safegaurding and LOP"
#Intersect and Dissolve the New Safegaurding Area
arcpy.Intersect_analysis([fc1,fc6],fc30)
arcpy.Dissolve_management(fc30, fc31,'OwnershipReferenceNumber')

print "Updating Old Safegaurding and LOP"
#Intersect and Dissolve the Old Safegaurding Area
arcpy.Intersect_analysis([fc1,fc7],fc32)
arcpy.Dissolve_management(fc32, fc33,'OwnershipReferenceNumber')

JoinFields = ['OwnershipReferenceNumber','OBJECTID']
Update_Fields = ['OwnershipReferenceNumber','Include']
# Use list comprehension to build a dictionary from a da SearchCursor
valueDict = {r[0]:(r[1:]) for r in arcpy.da.SearchCursor(fc31,JoinFields)}
valueDict1 = {r[0]:(r[1:]) for r in arcpy.da.SearchCursor(fc33,JoinFields)}
...
print"Updating Unique Field for LOP"
#Update the fields holding the global ID with a cursor
...
print "Extract HMLR for safeguaridng"
arcpy.Select_analysis(fc2, fc4,"""Include = 'Yes'""")
arcpy.Select_analysis(fc4, fc5)

print "Extract LOP for safeguaridng"
arcpy.Select_analysis(fc1, fc20,"""Include = 'Yes'""")
arcpy.Select_analysis(fc20, fc21)
...
for row in arcpy.da.SearchCursor(fc3, ['Hmlr_GlobalID']):
...
arcpy.Intersect_analysis([fc1,fc4],fc13)
...
joinfields = ['HMLR_Title_No','HMLR_Unique_ID']
arcpy.JoinField_management(fc1,"LOP_Unique_ID",fc18,"LOP_Unique_ID",joinfields)
arcpy.Dissolve_management(fc1,fc15,"HMLR_Title_No")
arcpy.Select_analysis(fc15,fc25,"""HMLR_Title_No IS NOT NULL""")
...
arcpy.Select_analysis(fc21, fc16,"""ExternalReference LIKE 'U%'""")
...
with arcpy.da.InsertCursor(fc25, Fieldlist) as cursor:
    for row in Unumbers:
        cursor.insertRow(row)

#Add a Field for an update Cursor
arcpy.AddField_management(fc25, 'OwnershipReferenceNumber', "SHORT","","","", "", "", "", "")
